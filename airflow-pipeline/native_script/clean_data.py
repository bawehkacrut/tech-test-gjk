...
def get_file(url):
    ...
    if resp.status_code == 200:
        directories = 'result/number_2/'
        dataset_path = 'result/number_2/dataset.zip'

        if not os.path.exists(directories):
            os.makedirs(directories) 

        logging.info("Download File Success")
        open(dataset_path, 'wb').write(resp.content)

        # unzip downloaded file
        with ZipFile(dataset_path, 'r') as zipObj:
            # Extract all the contents of zip file
            zipObj.extractall(directories)

def write_to_csv(df):
    filename = 'result/number_2/result_2.csv'

    logging.info("Process: Will write CSV file to directory: {}".format(filename))
    df.to_csv(path_or_buf=filename, index=False)
    logging.info("Process: Write CSV file to directory: {} Success!".format(filename))
...
if __name__ == '__main__':
    ...
    target_directory = "result/number_2"
    root = "result/number_2/dataset"
